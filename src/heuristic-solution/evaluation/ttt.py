...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'directory', help='Path to the directory containing CSV files')

    args = parser.parse_args()

    csv_files = [os.path.join(args.directory, f) for f in os.listdir(
        args.directory) if f.endswith('.csv')]
    ...
    if not csv_files:
        ...
        for csv_file in csv_files:
            print(csv_file)
            output = os.path.join(args.directory, os.path.splitext(
                os.path.basename(csv_file))[0] + '_plot.eps')
